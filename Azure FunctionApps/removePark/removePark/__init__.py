...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if(username and password):
        try:
            ...
            state = "no park"
            location = updateDB.getLocation(username)
            if(location != None):
                updateDB.removePark(username, location)
                state = "park removed"
            return func.HttpResponse(json.dumps({"res": "ok", "state": state}), mimetype="application/json")
        except:
